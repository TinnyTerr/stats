 ...
 * ss, systemctl and the Docker socket — so these are what ship.
 *
...
const LINUX_TARGETS: Target[] = [
  { target: "bun-linux-x64", asset: "stats-linux-x64", note: "glibc, x86-64 with AVX2" },
  {
  ...
  { target: "bun-linux-arm64-musl", asset: "stats-linux-arm64-musl", note: "musl (Alpine), arm64" },
];

/**
 * Built only with --all. The hub half runs fine on macOS, but every collector
 * is Linux-specific, so a macOS box can only be a hub watching remote agents —
 * never a `driver: "local"` entry.
 */
const EXTRA_TARGETS: Target[] = [
  { target: "bun-darwin-arm64", asset: "stats-darwin-arm64", note: "macOS Apple Silicon (hub only)" },
  { target: "bun-darwin-x64", asset: "stats-darwin-x64", note: "macOS Intel (hub only)" },
];
...
function hostAsset(): string {
  ...
  const musl = Bun.file("/lib/ld-musl-x86_64.so.1").size > 0 ||
...
async function smokeTest(path: string) {
  const proc = Bun.spawn([resolve(path), "version"], { stdout: "pipe", stderr: "pipe" });
  const [out, code] = await Promise.all([new Response(proc.stdout).text(), proc.exited]);
  if (code !== 0 || !out.includes(pkg.version)) {
    throw new Error(`smoke test failed for ${path}: exit ${code}, output ${JSON.stringify(out)}`);
  }
...
console.log(`stats ${pkg.version} — building ${targets.length} target(s) into ${outDir}/\n`);
