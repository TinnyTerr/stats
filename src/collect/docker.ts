...
async function containerStats(
	...
	try {
		const res = await dockerFetch(
			`/containers/${id}/stats?stream=false&one-shot=false`,
		);
...
export async function collectContainers(
	...
	if (!res.ok) {
		throw new DockerUnavailable(
			`docker returned ${res.status} listing containers`,
		);
	}
...
/**
 * start/stop/restart, straight against the engine API. Only reachable when the
 * node is started with control enabled; the socket's own permissions are the
 * second gate.
 */
export async function containerAction(
	...
	const res = await dockerFetch(
		`/containers/${encodeURIComponent(target)}/${verb}`,
		{
			method: "POST",
		},
	);
	// 304 means it was already in that state, which is a no-op, not a failure.
	if (res.ok || res.status === 304) {
		return {
			ok: true,
			output: `${verb} ${target}: ${res.status === 304 ? "already done" : "ok"}`,
		};
	}
	...
	return {
		ok: false,
		output: body?.message ?? `docker returned ${res.status}`,
	};
}
...
export async function containerLogStream(
	...
	const res = await dockerFetch(
		`/containers/${encodeURIComponent(target)}/logs?${params}`,
		{
			signal: opts.signal,
		},
	);
	if (!res.ok || !res.body) {
		throw new DockerUnavailable(
			`docker returned ${res.status} for logs of ${target}`,
		);
	}
	return {
		body: res.body,
